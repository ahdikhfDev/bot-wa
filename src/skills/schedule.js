 ...
 * - /schedule digest [HH:MM] — auto IT content (GitHub+HN+Dev.to+arXiv)
...
export default {
    name: 'schedule',
    title: 'Scheduler',
    description: 'Atur job otomatis (berita, pengingat, digest)',
    ...
    async handler(sock, remoteJid, args, context) {
            ...
            for (const job of jobs) {
                ...
                switch (job.action_type) {
                    ...
                    case 'search_news':
                        ...
                        break;
                    case 'it_digest':
                        info = `📡 IT Digest (GitHub+HN+Dev.to+arXiv)`;
                        break;
                    case 'prayer_tracker':
        ...
        if (subcommand === 'digest') {
            ...
            if (!timeStr) {
                await sock.sendMessage(remoteJid, {
                    text: `❌ Usage: /schedule digest [HH:MM]\n\nContoh:\n/schedule digest 06:30\n/schedule digest 17:00`
                });
                return;
            }
            const [h, m] = timeStr.split(':').map(Number);
            if (isNaN(h) || isNaN(m) || h < 0 || h > 23 || m < 0 || m > 59) {
                await sock.sendMessage(remoteJid, { text: '❌ Format waktu salah. Gunakan HH:MM (contoh: 06:30)' });
                return;
            }
            const jobName = `digest_${remoteJid.replace(/[^a-zA-Z0-9]/g, '_')}_${h}_${m}`;
            addJob(jobName, remoteJid, h, m, 'it_digest', {});
            await sock.sendMessage(remoteJid, {
                text: `✅ *Auto IT Digest Aktif!* 📡\n⏰ Setiap hari jam ${formatTime(h, m)} WIB\n📡 Konten: GitHub Trending + HackerNews + Dev.to + arXiv Papers\n\nBot akan otomatis kirim kompilasi IT trending ke sini.`
            });
            return;
        }

        await sock.sendMessage(remoteJid, {
            text: `📋 *Schedule Commands* 📋\n\n• /schedule list — lihat semua job\n• /schedule add [nama] [HH:MM] [pesan]\n  Contoh: /schedule add pagi 06:00 Selamat pagi\n• /schedule news [HH:MM] [topik]\n  Contoh: /schedule news 06:00 berita terkini\n• /schedule digest [HH:MM] — IT trending otomatis\n  Contoh: /schedule digest 06:30\n• /schedule remove [nama] — hapus job\n• /schedule toggle [nama] — hidup/matiin job\n• /setjadwalsholat [kota] — notif sholat otomatis\n• /unsetjadwalsholat — matiin notif sholat`
        });
    }
};
