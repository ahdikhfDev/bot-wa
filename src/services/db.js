...
export function createStock({ topic, caption, tags, videoPath, thumbnailPath, videoSize, trendSource }) {
    if (!db) return null;
    db.run(
        'INSERT INTO stock (topic, caption, tags, video_path, thumbnail_path, video_size, trend_source) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [topic || '', caption || '', JSON.stringify(tags || []), videoPath || '', thumbnailPath || '', videoSize || 0, trendSource || '']
    );
...
export function getStocks(limit = 50, offset = 0, status = '') {
    if (!db) return [];
...
export function getStockById(id) {
    if (!db) return null;
    const r = db.exec('SELECT * FROM stock WHERE id = ?', [id]);
    if (!r.length || !r[0].values.length) return null;
    const cols = r[0].columns;
    const row = r[0].values[0];
    const obj = {};
...
export function updateStock(id, updates) {
    ...
    for (const [key, val] of Object.entries(updates)) {
        ...
        } else if (['topic', 'caption', 'video_path', 'thumbnail_path', 'video_size', 'status', 'trend_source', 'scheduled_at'].includes(col)) {
            fields.push(`${col} = ?`);
            params.push(val);
        }
    }
    ...
    params.push(id);
    db.run(`UPDATE stock SET ${fields.join(', ')} WHERE id = ?`, params);
    const changes = db.getRowsModified();
    saveDb();
    return changes > 0;
}
...
export function getStockCount(status = '') {
    if (!db) return 0;
    let sql = 'SELECT COUNT(*) as count FROM stock';
    const params = [];
    if (status) { sql += ' WHERE status = ?'; params.push(status); }
    const r = db.exec(sql, params);
